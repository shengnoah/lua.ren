...
首先，我们在ida中分析lua引擎tmgs.dll文件，然后定位到luaV_execute函数（搜索字符串“ ‘for’ limit must be a
...
---|---  
![](https://litna.top//2018/07/08/浅析android手游lua脚本的加密与解密（后续）/1.png) |
![](https://litna.top//2018/07/08/浅析android手游lua脚本的加密与解密（后续）/2.png)  
...
测试发现出错了，出错结果：  
![](https://litna.top//2018/07/08/浅析android手游lua脚本的加密与解密（后续）/3.png)

从出错的结果可以看出是luac文件的版本号有错误，这里无法识别lua
11的版本其实是题目故意设计让工具识别错误，我们将文件的第4个字节（lua版本号）11修改成53就可以了。正确结果：  
![](https://litna.top//2018/07/08/浅析android手游lua脚本的加密与解密（后续）/4.png)
